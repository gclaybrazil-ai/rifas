    ...
    try {
        ...
        if(data.success && data.rifas) {
            ...
            data.rifas.forEach(rifa => {
                ...
                // Formatar preco (ex: de 13.00 para 13 e 00 p/ estilizar)
                ...
                const colorPulse = rifa.tag.includes('AO VIVO') ? 'animate-pulse bg-red-600' : 'bg-blue-600';

                const cardHTML = `
                <a href="rifa.html" class="block relative w-full h-[28rem] rounded-[2rem] overflow-hidden shadow-xl shadow-gray-200 group transition-transform active:scale-95 bg-black cursor-pointer border-4 border-white">
                    <img src="${rifa.imagem_url}" class="absolute inset-0 w-full h-full object-cover opacity-70 group-hover:opacity-100 transition-opacity duration-300" alt="${rifa.nome}">
