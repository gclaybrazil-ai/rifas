...
const els = {
    ...
    alertIconError: document.getElementById('alert-icon-error'),
    alertIconInfo: document.getElementById('alert-icon-info'),
...
function toggleSelection(numero, serverStatus) {
    ...
    if (state.selecionados.has(numero)) {
        ...
        if (state.selecionados.size >= 20) {
            showAlert('Você pode selecionar no máximo 20 números.');
            return;
        }
...
function showAlert(message, title = 'Atenção', type = 'error') {
    ...
    if (type === 'error') {
        els.alertIconError.classList.remove('hidden');
        els.alertIconInfo.classList.add('hidden');
    } else {
        els.alertIconError.classList.add('hidden');
        els.alertIconInfo.classList.remove('hidden');
    }

    openModal(els.modalAlert);
}

els.btnCloseAlert.addEventListener('click', hideModals);
...
els.btnSubmitReservation.addEventListener('click', async () => {
    ...
    if (!nome || !whatsapp) {
        showAlert('Preencha seu nome e whatsapp!');
        return;
    }

    const partesNome = nome.split(/\s+/);
    if (partesNome.length < 2 || partesNome[1].length < 2) {
        showAlert('Por favor, informe seu NOME e SOBRENOME para continuar.');
        return;
    }
    ...
    if (whatsNum.length < 10) {
        showAlert('Por favor, informe um WhatsApp válido com DDD.');
        return;
    }

    if (arr.length === 0) {
        showAlert('Selecione ao menos um número.');
        hideModals();
        return;
    }
