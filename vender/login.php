...
<head>
    ...
    <title>Login do Criador - $UPER$ORTE</title>
...
<body class="bg-[#f8fafc] min-h-screen flex flex-col antialiased">
        ...
        <div class="w-full max-w-md">
            ...
            <div class="text-center mb-10">
                <h1 class="text-3xl font-black text-[#2c3e50] tracking-tighter uppercase italic leading-[0.9] mb-4">
                    Bem-vindo de <br> <span class="gradient-text">Volta ao SaaS.</span>
                </h1>
                <p class="text-gray-500 font-medium text-sm">Acesse seu painel administrativo de criador.</p>
            </div>
            ...
            <div class="bg-white rounded-[2.5rem] shadow-2xl p-8 border border-gray-100 relative overflow-hidden">
                ...
                <form id="loginForm" class="space-y-4 relative z-10">
                    <div>
                        <label class="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1.5 block">E-mail ou Usuário</label>
                    ...
                    <div>
                        <div class="flex justify-between items-center mb-1.5">
                            <label class="text-[10px] font-black text-gray-400 uppercase tracking-widest block">Senha</label>
                            <a href="#" class="text-[9px] font-black text-green-600 uppercase tracking-widest">Esqueceu?</a>
                        </div>
            ...
            <p class="text-center text-gray-400 text-[10px] mt-8 font-black uppercase tracking-widest">
                Ainda não tem conta? <a href="register.php" class="text-green-600 hover:text-green-700 underline">Começar Agora</a>
            </p>
        </div>
    </main>
    ...
    <script>
        document.getElementById('loginForm').addEventListener('submit', async (e) => {
            ...
            msg.innerText = 'AUTENTICANDO NO BANCO SEPARADO...';

            try {
                const response = await fetch('backend/api/login_usuario.php', {
                    method: 'POST',
                    body: formData
                });
                ...
                if (data.success) {
                    ...
                    msg.innerText = 'SUCESSO! ACESSANDO PAINEL...';
                    
                    const target = (data.role === 'admin') ? 'admin/index.php' : 'dashboard.php';
                    setTimeout(() => window.location.href = target, 1500);
                } else {
                    msg.className = 'text-center text-xs font-bold text-red-500 tracking-widest uppercase';
                    msg.innerText = data.error || 'Login inválido no SaaS.';
                    btn.disabled = false;
